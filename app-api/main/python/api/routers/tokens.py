...
def enable_token(token_id: str, db: Session = Depends(get_db)):
    ...
    if not db_token:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Token not found")
    return tokens.set_token_enabled_status(db=db, db_token=db_token, enabled=True)
...
def disable_token(token_id: str, db: Session = Depends(get_db)):
    ...
    if not db_token:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Token not found")
    return tokens.set_token_enabled_status(db=db, db_token=db_token, enabled=False)
...
def update_token(token_id: str, token_update: schemas.TokenUpdate, db: Session = Depends(get_db)):
    ...
    if not db_token:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Token not found")
    return tokens.update_token(db=db, db_token=db_token, token_update=token_update)
...
def delete_token(token_id: str, db: Session = Depends(get_db)):
    ...
    if not db_token:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Token not found")
    tokens.delete_token(db=db, db_token=db_token)