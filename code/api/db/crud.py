...
def set_default_provider(db: Session, db_provider: Provider):
...
def delete_provider(db: Session, db_provider: Provider):
...
def get_enabled_tokens(db: Session, skip: int = 0, limit: int = 100):
    return db.query(Token).filter_by(is_enabled=True).offset(skip).limit(limit).all()
