...
async def fetch_prices(provider: Provider, tokens: List[Token]) -> List[Dict[str, str]]:
    """
    ...
    for token in tokens:
        ...
        logger.info(f"Fetched price for {token.symbol} from {provider.name}: {simulated_price}")
        fetched_data.append({
            "token": token,
